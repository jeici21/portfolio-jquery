...
const FormacionYExp = () => {
  ...
  const checkFormacionYExpVisibility = () => {
    if (($(window).scrollTop() || 0 / 2) + ($(window).height() || 0) > ($('#visible-form-y-exp').offset()?.top || 0)) {
      $('#visible-form-y-exp').removeClass(`opacity-0 invisible ${fadeinDiv(false)}`).addClass(`opacity-100 visible ${fadeinDiv(true)}`);
    } else {
      $('#visible-form-y-exp').removeClass(`opacity-100 visible ${fadeinDiv(true)}`).addClass(`opacity-0 invisible ${fadeinDiv(false)}`)
    }
  }
